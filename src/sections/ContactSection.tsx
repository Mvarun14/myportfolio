...
export default function ContactSection() {
  ...
  try {
      const response = await fetch('/api/contact/send', {
        ...
        body: JSON.stringify(formData), // only the form data
      });

      const data = await response.json();

      if (data.success) {
        toast.success('Message sent successfully'); // or use your custom success message
        setFormData({ name: '', email: '', message: '' }); // optional: reset form
      } else {
        toast.error('Failed to send message'); // or use your custom error message
      }
    } catch (error) {
